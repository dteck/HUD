#------
# Housing and Urban Development (HUD) Crosswalk API Interface
...
HUD_tract2zip<- function(GEOID){ #custom function to convert tracts to zip codes
  ...
  library(stringr) #library to sanitize strings
  tract_san<- paste(str_trim(as.character(GEOID), side = "both")) #sanitizes input track number
  URL<-paste("https://www.huduser.gov/hudapi/public/usps?type=6&query=", tract_san, sep="") #build URL
